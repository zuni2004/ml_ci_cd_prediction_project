"""
CI/CD BUILD FAILURE PREDICTOR - RESULTS ANALYSIS & MODEL LOADING

This utility module provides functions to:
    1. Load saved trained models from pickle files
    2. Load and analyze training logs
    3. Display detailed training metrics
...
def print_training_summary(logs: Dict[str, Any]) -> None:
    ...
    for metric in metrics:
        ...
        print(f"{metric.capitalize():<15} {test_mean:<12.4f} {test_std:<12.4f} {train_mean:<12.4f}")
    ...
    for idx, trial in enumerate(trials[:5], 1):
        ...
        params_str = ", ".join([f"{k}={v}" for k, v in list(trial['params'].items())[:2]])
