...
def print_baseline_results(results_list: List[Dict[str, Any]]) -> None:
    ...
    for results in results_list:
        print("{:<20} {:<12.2f} {:<12.4f} {:<12.4f} {:<12.4f}".format(
            results["model_name"], 
...
def select_best_model(
    ...
    log_message(f"Best model selected: {best_model_name} (F1: {best_score:.4f})", "SUCCESS")
...
def create_optuna_study(
    ...
    def objective(trial: optuna.Trial) -> float:
        if model_name == "Logistic Regression":
            ...
            model = LogisticRegression(C=C, max_iter=max_iter, random_state=CONFIG["random_seed"], n_jobs=-1, solver="saga")
        elif model_name == "Decision Tree":
            max_depth = trial.suggest_int("max_depth", 3, 30)
            min_samples_split = trial.suggest_int("min_samples_split", 2, 100)
            min_samples_leaf = trial.suggest_int("min_samples_leaf", 1, 50)
            criterion = trial.suggest_categorical("criterion", ["gini", "entropy"])
            model = DecisionTreeClassifier(max_depth=max_depth, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, criterion=criterion, random_state=CONFIG["random_seed"])
            ...
            model = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, max_features=max_features, random_state=CONFIG["random_seed"], n_jobs=-1)
        
        model.fit(X_train, y_train)
        y_pred = model.predict(X_test)
        return f1_score(y_test, y_pred, zero_division=0)
    
    study = optuna.create_study(direction="maximize", sampler=TPESampler(seed=CONFIG["random_seed"]), pruner=MedianPruner())
    ...
    study_history = {
        ...
        "trials_history": [{"trial_number": t.number, "value": t.value, "params": t.params} for t in study.trials],
    }
    return best_params, study_history

def train_final_model(model_name: str, best_params: Dict[str, Any], X_train: np.ndarray, y_train: np.ndarray) -> Any:
    log_message(f"Training final {model_name} with best hyperparameters...")
    start_time = time.time()
    
    if model_name == "Logistic Regression":
        model = LogisticRegression(**best_params, random_state=CONFIG["random_seed"], n_jobs=-1, solver="saga")
    elif model_name == "Decision Tree":
        model = DecisionTreeClassifier(**best_params, random_state=CONFIG["random_seed"])
    elif model_name == "Random Forest":
        model = RandomForestClassifier(**best_params, random_state=CONFIG["random_seed"], n_jobs=-1)
    
    model.fit(X_train, y_train)
    log_message(f"✓ Final model trained in {time.time() - start_time:.2f}s", "SUCCESS")
    return model

def evaluate_final_model(model: Any, model_name: str, X_test: np.ndarray, y_test: np.ndarray) -> Dict[str, Any]:
    ...
    metrics["confusion_matrix"] = {"true_negatives": int(tn), "false_positives": int(fp), "false_negatives": int(fn), "true_positives": int(tp)}
...
def save_results(model, model_name, best_params, cv_results, final_metrics, optuna_history):
    ...
    logs = {"timestamp": timestamp, "model_name": model_name, "final_test_metrics": final_metrics, "best_hyperparameters": best_params, "optuna_history": optuna_history}
...
def main():
    ...
    try:
        # STEP 1: Load Data
        X_train, X_test, y_train, y_test = load_data(CONFIG["train_file"], CONFIG["test_file"], CONFIG["target_col"])
        ...
        for model_name, model in baseline_models.items():
            res = evaluate_model_single_run(model, X_train, y_train, X_test, y_test, model_name)
        ...
        best_params, optuna_history = create_optuna_study(best_model_name, X_train, y_train, X_test, y_test, n_trials=30)
        
        # STEP 6: Train Final Model
        final_model = train_final_model(best_model_name, best_params, X_train, y_train)
        
        # STEP 7: Evaluate Final Model
        final_metrics = evaluate_final_model(final_model, best_model_name, X_test, y_test)
        
        # STEP 8: Save Results
        save_results(final_model, best_model_name, best_params, best_res, final_metrics, optuna_history)
