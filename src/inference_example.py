...
def load_trained_model(
    model_path: str = "src/saved_models/best_model_Random_Forest.pkl",
):
    """
...
def load_encoders_and_scaler(
    ...
    try:
        ...
        print(f"  Target encoder not found (may not be needed for inference)")
        target_encoder = None
    ...
    try:
        ...
        print(f"  MinMax scaler not found")
        minmax_scaler = None
...
def interpret_prediction(prediction_dict: Dict[str, Any]) -> str:
    ...
    if confidence > 0.9:
        ...
        interpretation += (
            f"  • Assessment      : Moderate confidence, review recommended\n"
        )
    else:
        interpretation += (
            f"  • Assessment      : Low confidence, manual review needed\n"
        )
...
def main():
    ...
    try:
        ...
        print(
            f"  • Predicted failures    : {batch_result['prediction_counts']['failures']}"
        )
        print(
            f"  • Predicted passes      : {batch_result['prediction_counts']['passes']}"
        )
        print(f"  • Mean confidence       : {batch_result['mean_confidence']:.1%}")
        print(
            f"  • Pass rate             : {batch_result['prediction_counts']['passes']/len(X_test):.1%}"
        )
        ...
        print("-" * 70)
        print(
            f"{'Sample':<8} {'Prediction':<12} {'P(Fail)':<12} {'P(Pass)':<12} {'Confidence'}"
        )
        print("-" * 70)
        ...
        for idx in range(len(X_test)):
            ...
            print(
                f"{idx:<8} {label:<12} {p_fail:<12.4f} {p_pass:<12.4f} {confidence:>10.1%}"
            )
...
if __name__ == "__main__":
    ...
    if [ "$RESULT" == "FAIL" ]; then
        echo "  Prediction: This build is likely to FAIL"
